...
>/.tip={Latex[length=7mm, width=.5mm]}]
	...
	\draw[track] (blockout) to (block.east);
	
	\draw[track] (tactin) to[out=180,in=0] (shuntout);
	\draw[track] (opin) to[out=180,in=0] (shuntout);
	
	\draw[track] (shu.east) to[out=0,in=180] (shuntout);
	
	\draw[track] (tactin) to[out=180,in=0] (rosto.east);
	\draw[track] (opin) to[out=180,in=0] (rosto.east);
	\draw[track] (tactin) to[out=180,in=0] (cscdout);
	\draw[track] (opin) to[out=180,in=0] (cscdout);
	\draw[track] (cscdout) to (cscd.east);
	%operational
